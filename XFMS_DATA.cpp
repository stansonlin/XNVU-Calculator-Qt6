...
// 把所有 QRegularExpression 替换为 QRegularExpression
...
int XFMS_DATA::_loadXP10(const QString& file, int type)
{
    ...
    while(!infile.atEnd())
	{
        ...
        QStringList list;
        if(type == 3 || type == 5) list = line.split('|');//, Qt::SkipEmptyParts);                 //RSBN data (rsbn.dat) and xnvu_wps.txt
        ...
        try
        {
            for(unsigned int i=0; i<record.size(); i++)
            {
                ...
                case 9: //Top of airway FL
                    ...
                    break;
                case 10:
                    //aSeg.sharedAirways = qstr.split('-', Qt::SkipEmptyParts);
